...
namespace WalkNWash.VRCompanion
{
    internal sealed class OpenXrInput : IControllerInput
    {
        ...
        private ulong set, moveAction, turnAction;
        ...
        internal OpenXrInput(ulong instance, ulong session, Action<string> log)
        {
            ...
            try
            {
                ...
                moveAction = Create("move", "Move");
                turnAction = Create("turn", "Snap turn");
                ...
                foreach (string profile in profiles)
                {
                    ...
                    {
                        var suggested = new SuggestedBindings { type = 51, profile = Path("/interaction_profiles/" + profile), count = 2, bindings = bindings.Pointer };
        ...
        private ulong Create(string name, string label)
        {
            var info = new ActionInfo { type = 29, name = name, localizedName = label, actionType = 3 };
        ...
        public void Poll(out Vector2 move, out Vector2 turn)
        {
