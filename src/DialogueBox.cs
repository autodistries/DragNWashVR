...
{
    // Rounded warm speech-panel silhouette with a crisp dark outline.
    internal sealed class DialogueBox : Image
    {
        protected override void OnPopulateMesh(VertexHelper mesh)
        {
            mesh.Clear();
            var rect = rectTransform.rect;
            Fill(mesh, rect, new Color(0, 0, 0, color.a), true);
            rect.xMin += 2; rect.xMax -= 2; rect.yMin += 2; rect.yMax -= 2;
            Fill(mesh, rect, color);
        }
        private static void Fill(VertexHelper mesh, Rect rect, Color tint, bool border = false)
        {
            int start = mesh.currentVertCount;
            float radius = Mathf.Min(22, Mathf.Min(rect.width, rect.height) * .25f);
