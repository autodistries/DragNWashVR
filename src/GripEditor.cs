...
namespace WalkNWash.VRCompanion
{
    internal sealed class GripEditor : IDisposable
    {
        ...
        {
            if (!DebugAccess.Enabled || !usable) { if (Active) Finish(false); return; }
            ...
            if (!Active) return;
            backend.Poll();
            if (!host.TryHandFrame(host.FreeHand, out var free) || !host.TryHandFrame(host.ToolHand, out var toolFrame))
            { Finish(false); return; }
            ...
            bool down = grab.Update(host.Roles.FreeTrigger(backend.LeftTrigger, backend.RightTrigger), true);
            float scale = host.HandScale;
            if (down)
            {
                if (!dragging)
                {
                    var pose = grips.Apply(tool, toolFrame, scale);
                    grabPosition = Quaternion.Inverse(free.Rotation) * (pose.Position - free.Position);
                    grabRotation = Quaternion.Inverse(free.Rotation) * pose.Rotation;
                }
                Vector3 target = free.Position + free.Rotation * grabPosition;
                position = Quaternion.Inverse(toolFrame.Rotation) * (target - toolFrame.Position) / scale;
                rotation = (Quaternion.Inverse(toolFrame.Rotation) * free.Rotation * grabRotation).eulerAngles;
            }
            dragging = down;
            if (!down)
            {
                bool depth = host.Roles.ToolTrigger(backend.LeftTrigger, backend.RightTrigger) > .65f;
        ...
        internal void BeginEye(Backend backend)
        {
            ...
            text.text = "Grip: " + tool + "\n" + host.Roles.FreeName + " trigger: grab / release pose\nSticks: 1 mm XY / 1 degree yaw-pitch\nHold " + host.Roles.ToolName + " trigger: depth / roll\nRight A: SAVE   Right B / F8: CANCEL   Left X: RESET\n"
