...
internal static class Checks
{
    ...
    private static int Main(string[] args)
    {
        ...
        Assert(HudToggleBinding.OpenXrPath("oculus/touch_controller") == "/user/hand/left/input/y/click", "Touch left Y toggles HUD");
        Assert(HudToggleBinding.OpenXrPath("valve/index_controller") == "/user/hand/left/input/b/click", "Index left secondary face button toggles HUD");
        Assert(HudToggleBinding.OpenXrPath("htc/vive_controller") == null, "Vive profile gets no invalid Y binding");
        Assert(HudToggleBinding.OpenXrPath("microsoft/motion_controller") == null, "motion profile gets no invalid Y binding");
        Assert(HudToggleBinding.OpenVrPressed(1, 1UL << 1), "OpenVR left Y mapping");
        Assert(!HudToggleBinding.OpenVrPressed(2, 1UL << 1), "right B does not toggle HUD");
        Assert(!HudToggleBinding.OpenVrPressed(1, 1UL << 7), "left X does not toggle HUD");
        Assert(!HudToggleBinding.OpenVrPressed(1, (1UL << 33) | (1UL << 2)), "triggers and grip do not toggle HUD");
        var hudToggle = new HudToggle();
        Assert(!hudToggle.Update(true, true), "Y held at startup ignored");
        Assert(!hudToggle.Update(false, true), "Y release arms without toggling");
        Assert(hudToggle.Update(true, true), "Y press toggles once");
        Assert(!hudToggle.Update(true, true), "held Y does not repeat");
        Assert(!hudToggle.Update(false, true), "Y release does not toggle");
        Assert(hudToggle.Update(true, true), "second Y press toggles back");
        Assert(!hudToggle.Update(true, false), "disabled input cannot toggle");
        Assert(!hudToggle.Update(true, true), "Y held across focus return ignored");
        hudToggle.Update(false, true);
        Assert(hudToggle.Update(true, true), "Y works again after focus return and release");
